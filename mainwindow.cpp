...
double MainWindow::getPxMoreY(const unsigned int k, std::vector<double> const &pX, std::vector<double> const &pY)
{
    ...
    for(unsigned int i(0) ; i <= k ; ++i)
     ...
     //pYx[0] utilisé comme pBx[0] est modifié par cette fonction
    //puis cette valeur modifiée est utilisée pour le calcul suivant -> faux!
    //fixed <3
...
unsigned int MainWindow::setN()
{
    ...
    if(ui->pacifist->isChecked() || ui->crasyEye->isChecked()) //si pacifistes et/ou oeil fou checked, on divise le nbre de PA dispos
         var /= 3;
    else if(ui->pacifist->isChecked() && ui->crasyEye->isChecked())
         var /= 5;
...
void MainWindow::setWeaponsStuff(std::vector<double> &pBk, std::vector<double> &pLk, std::vector<double> &pNk,
                                 std::vector<double> &pGk, std::vector<double> &pRk,
                                 std::vector<double> &pYk, double &eB, double &eL, double &eR, double &eG, double &eN,
                                 unsigned int &chargesUsed, const unsigned int shooterPa, const unsigned int n, double &damages,
                                  const double p)
{
